...
- AI Compare admin untuk menguji satu module dengan 2-4 model AI sekaligus, melihat status, durasi, token, cost Rupiah, hasil berdampingan, dan rekomendasi opsional dari AI penilai
...
Module dikelola melalui admin dengan alur **Draft → Preview → Publish** untuk playbook. Module baru tidak dapat dipilih bot sebelum playbook dipublikasikan dan aksesnya dicentang pada membership. Memilih `/company` mereset module ke `General`; mengganti module tidak menghapus history lama, tetapi memakai ruang history yang terpisah. Checkbox **Gunakan Knowledge perusahaan** menentukan apakah module ikut memuat published Knowledge company. Default aktif untuk menjaga perilaku lama; matikan untuk module yang playbook-nya mandiri agar prompt lebih ringan.
...
**AI Compare** tersedia di admin untuk membandingkan 2-4 model. Mode **Pilih modul** memakai module perusahaan atau modul bersama; untuk module perusahaan, playbook dan instruction company ikut dipakai, sedangkan knowledge company hanya ikut bila checkbox module aktif. Mode **Prompt sendiri** hanya memakai isi textbox sebagai prompt dan mengabaikan module, instruction, dan knowledge perusahaan. Hasil tampil berdampingan per model: status, durasi, token, estimasi cost USD/Rupiah dengan kurs tetap Rp 17.500, preview hasil, dan jawaban penuh. Admin juga dapat memilih satu **AI penilai** opsional untuk membaca ringkasan hasil compare dan memberi rekomendasi 2 paragraf berdasarkan kualitas, durasi, token, dan biaya. Hasil compare tidak disimpan permanen; Log Admin hanya mencatat metadata singkat. Modul Learning diuji lewat alur khusus buku aktif karena kualitasnya bergantung pada retrieval isi PDF, bukan hanya prompt module.
